    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="min-h-screen bg-[#121212] text-white flex items-center justify-center p-6">
            <div class="w-full max-w-2xl bg-[#181818] p-8 rounded-xl shadow-2xl">
                <h1 class="text-3xl font-bold mb-8">Créer une playlist</h1>
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="flex flex-col sm:flex-row gap-6 items-start">
                         <div class="w-40 h-40 bg-[#282828] shrink-0 rounded shadow-inner flex items-center justify-center text-5xl text-gray-600 relative overflow-hidden group">
                             <span v-if="!form.image">♫</span>
                             <img v-else :src="form.image ? URL.createObjectURL(form.image) : ''" class="absolute inset-0 w-full h-full object-cover" />
                    ...
                    <div class="flex justify-end pt-4">
                        <Link href="/playlists" class="px-6 py-3 font-bold text-gray-400 hover:text-white transition mr-4">Annuler</Link>
                        <button
                            type="submit"
